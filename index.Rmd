...
* NAMESPACE file which specifies the function that compose your package and what imports functions from other packages are needed.
...
```{r, echo=T, out.width="70%", warning=FALSE}
...
`@export` informs Roxygen to put the function name in the package NAMESPACE file which means it can be accessed by users after they run `library(<packagename>)`.
