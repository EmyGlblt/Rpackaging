...
&rarr; If you need to tailor some existing functions, are going to use them many times or want to save time and have a way to standardize your coding with existing one.
...
&rarr; Do you need a place to share code and make it available. Github is an option, but CRAN make it more accessible.
...
One of the main principles in good coding practice is 'do not repeat yourself'.
...
.blackbox[
.font60[
`#' @params n_col integer values.`  
`#' @params n_row integer values.`  
`#' @params plot. Plot the image if TRUE.`  
`#'`  
...
available::available("spatgrad", browse = F)
...
usethis::create_package("spatgrad")
...
**Don't forget to add the packages you use inside your functions to the dependencies section of DESCRIPTION file**
